...
pub async fn get_event_by_id(trx: &impl ConnectionTrait, id : Uuid) -> BackendResult<DbEvent::Model> {
    let event = DbEvent::Entity::find_by_id(id).one(trx).await?;
    match event {
        None => Err(BackendError::IdNotFound { entity: "event".to_string(), id: id.to_string() }),
        Some(event) => Ok(event),
    }
}


pub async fn add_event(trx: &impl ConnectionTrait, event: CreateEvent) ->  BackendResult<DbEvent::Model>  {
...
pub async fn update_event(trx: &impl ConnectionTrait, event: DbEvent::Model) -> BackendResult<DbEvent::Model> {
