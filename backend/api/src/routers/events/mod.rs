...
pub async fn get_technical_questions(
    ...
    State(state): State<ApiState>,
    Path(event_id): Path<Uuid>,
) -> ApiJsonVec<TechnicalQuestion> {
    let event = state.event_service.get_event(event_id).await?;
    let groups = Groups::from_event(ctx.roles(), event.id);

    if !groups.can_view_event_internal(event.visibility) {
        return Err(ApiError::Forbidden {
            action: "view technical questions".to_string(),
        });
    }
    let questions = state
        .rating_service
        .get_technical_questions(event_id)
        .await?;
    Ok(Json(questions))
}
...
pub async fn create_technical_questions(
    ...
    State(state): State<ApiState>,
    Path(event_id): Path<Uuid>,
    Json(body): Json<CreateTechnicalQuestionDTO>,
) -> ApiJson<TechnicalQuestion> {
    let event = state.event_service.get_event(event_id).await?;
    let groups = Groups::from_event(ctx.roles(), event.id);

    if !groups.can_manage_event() {
        return Err(ApiError::Forbidden {
            action: "create technical questions".to_string(),
        });
    }

    let create_question = CreateTechnicalQuestion {
        event_id,
    ...
    let questions = state
        .rating_service
        .create_technical_question(create_question)
        .await?;
    Ok(Json(questions))
}
...
pub async fn delete_technical_questions(
    ...
    if !groups.can_manage_event() {
        return Err(ApiError::Forbidden {
            action: "delete technical questions".to_string(),
        });
    }
    ...
    let rows_affected = AffectedRows {
        affected_rows: deleted_result.rows_affected,
    };
...
pub async fn update_technical_questions(
    ...
    State(state): State<ApiState>,
    Path((event_id, question_id)): Path<(Uuid, Uuid)>,
    Json(body): Json<UpdateTechnicalQuestionDTO>,
) -> ApiJson<TechnicalQuestion> {
    let event = state.event_service.get_event(event_id).await?;
    let groups = Groups::from_event(ctx.roles(), event.id);
    ...
    if !groups.can_manage_event() {
        return Err(ApiError::Forbidden {
            action: "update technical questions".to_string(),
        });
    }
