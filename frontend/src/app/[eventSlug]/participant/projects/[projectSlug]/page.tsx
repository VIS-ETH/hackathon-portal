...
const Editor = ({project} : {project: Project}) => {
    ...
    const [description, setDescription] = useState<string>(project?.content || "");
        ...
        <Stack mih={400} gap={"sm"}>
            <Flex justify={"end"} gap={"md"} align={"center"}>
                ...
                <Button loading={loading} onClick={() => {
                    ...
                    setLoading(true);
                    patch_query.mutate({content: description, name: title }, {
                        onError: (error) => {
                        ...
                        onSuccess: (data) => {
                            setLoading(false)
                            setError(null)
                            router.replace(`/${eventSlug}/participant/projects/${data.slug}`)
                        }
            ...
            <ProjectEditor title={title} setTitle={setTitle} description={description} setDescription={setDescription} preview={preview} />
        </Stack>
    )
}

export default function Page() {
    const { eventSlug: slug, projectSlug } = useParams<{ eventSlug: string, projectSlug: string }>();
