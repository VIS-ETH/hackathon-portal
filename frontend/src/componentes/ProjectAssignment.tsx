...
const unhappy_score = (
  team_preference: ProjectPreferences,
  project_id: string,
) => {
...
const TeamPreference = ({
  ...
  team: Team;
  projects: Project[];
  prov_project: string | undefined;
}) => {
  const { data: team_preference } = useGetTeamProjectPreferences(team.id);

  const id_project_mapping = projects.reduce(
    (acc, project) => {
  ...
  const [selected_project, setSelectedProject] = useState<string | null>(
    prov_project || null,
  );
  ...
  return (
    <Grid w={"100%"} align="center">
      ...
      <Grid.Col span={6}>
        <Group>
          {team_preference &&
            team_preference.project_preferences.map((pref, index) => (
              <>
      ...
      <Grid.Col span={1}>
        {team_preference &&
        team_preference.project_preferences.length > 0 &&
        selected_project ? (
          <Badge w={"100%"}>
            {unhappy_score(team_preference, selected_project)}
          </Badge>
        ) : (
        ...
        teams.map((team) => (
          <Group key={team.id}>
            <TeamPreference
              team={team}
              projects={projects}
              prov_project={project_assignment.get(team.id)}
            />
          </Group>
        ))}
    </Stack>
